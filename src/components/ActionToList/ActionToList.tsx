...
const ActionToList = (props: ActionToListProps) => {
  const { slug } = props;
  const toggleInList = useStore((state) => state.toggleInList);
  const getIsInList = useStore((state) => state.getIsInList);

  return (
    <button
      className={classnames(
        'w-full btn btn-sm flex gap-2',
        { 'btn-secondary': !getIsInList(slug) },
        { 'btn-outline': getIsInList(slug) }
      )}
      onClick={() => toggleInList(slug)}
    >
      {getIsInList(slug) ? (
        <>
          <svg
